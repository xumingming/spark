...
class InferFiltersFromConstraintsSuite extends PlanTest {
  ...
  test("correlated range predicate in left-outer join ON clause is pushed to right side " +
    ...
    // Simulates: SELECT * FROM a LEFT JOIN b ON a.key = b.key AND b.v >= a.k AND b.v <= a.k + 10
    // WHERE a.k = 5
    ...
    optimized.foreach {
      case Join(_, rightChild, LeftOuter, _, _) =>
  ...
  test("correlated range predicate in inner join ON clause is pushed to right side " +
    ...
    val originalQuery = left
      .where("a.k".attr === 5)
      .join(right, Inner, Some(joinCond))
      .analyze

    val optimized = Optimize.execute(originalQuery)

    optimized.foreach {
      case Join(_, rightChild, Inner, _, _) =>
        val rightFilters = rightChild.collect { case Filter(cond, _) => cond }
        val allConds = rightFilters.flatMap(splitConjunctivePredicates)
        assert(allConds.exists {
          case GreaterThanOrEqual(_: Attribute, Literal(v, IntegerType)) => v == 5
  ...
  test("correlated range predicate with non-deterministic expression is not inferred") {
    // Non-deterministic predicates in join ON conditions are rejected by the analyzer, so we
    // test the determinism guard at the unit level in ConstraintPropagationSuite.
    // Here we verify that no spurious filter is added when the substituted expression is
    // non-foldable (references a non-literal). We use a column from a third relation that is
    // not substituted, so the predicate still references right-side columns only after
    // substitution but produces an expression referencing an unresolved attribute.
    // The key correctness property - that non-deterministic results of substitution are
    // suppressed - is validated directly in ConstraintPropagationSuite.
  }
}
